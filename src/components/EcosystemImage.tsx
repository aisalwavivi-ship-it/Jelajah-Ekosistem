 ...
 * - Prevents raw browser broken image icons on 404/missing assets
 * - Avoids artificial white box backgrounds
 * - Preserves exact size, clickability, and layout positioning
 * - Uses clean educational vector placeholders when asset file is missing
 */
export const EcosystemImage: React.FC<EcosystemImageProps> = ({
  ...
  const renderLucideFallback = () => {
    ...
    } else if (lower.includes('kupu') || lower.includes('lebah') || lower.includes('ulat') || lower.includes('belalang')) {
      IconComponent = Bug;
      colorClass = 'text-amber-700 bg-amber-50/90 border-amber-300';
    } else if (lower.includes('ikan')) {
      IconComponent = Fish;
    ...
    return (
      <div
        ...
        title={`Asset ${alt} (memerlukan file ilustrasi)`}
      >
