...
export const GameOpeningScreen: React.FC<GameOpeningScreenProps> = ({
  ...
  // Progressive timed reveal of the opening game sequences (5-6 seconds total)
  useEffect(() => {
    const t1 = setTimeout(() => setPhase(2), 800);   // Scene 2: Title card
    const t2 = setTimeout(() => setPhase(3), 1800);  // Scene 3: Subtitle
    const t3 = setTimeout(() => setPhase(4), 2600);  // Scene 4: Mascot & Nature Elements
    const t4 = setTimeout(() => setPhase(5), 3400);  // Scene 5: Primary Action Call
    ...
    <AnimatePresence>
      {!isExiting && (
        <motion.div
          ...
          className="fixed inset-0 z-50 flex flex-col justify-between items-center w-full h-full overflow-hidden select-none"
        >
          ...
          <div className="w-full max-w-6xl mx-auto px-4 pt-4 sm:pt-6 flex items-center justify-between z-20">
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="group inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-white/85 hover:bg-white text-stone-700 hover:text-stone-900 border border-stone-200/80 shadow-md text-xs font-bold transition cursor-pointer backdrop-blur-md"
            >
              <span>Lewati</span>
          ...
          <div className="w-full max-w-3xl mx-auto px-4 py-4 flex flex-col items-center justify-center text-center z-20">
                  ...
                  <motion.button
                    ...
                    animate={{
                      boxShadow: [
                        ...
                        '0 12px 30px -2px rgba(16, 185, 129, 0.65)',
                        '0 10px 25px -5px rgba(5, 150, 105, 0.4)',
                      ],
                    }}
                    transition={{
                    ...
                    className="w-full py-4 px-8 bg-gradient-to-r from-emerald-500 via-emerald-600 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white font-display font-black text-lg sm:text-xl rounded-2xl shadow-xl border-2 border-emerald-300 flex items-center justify-center gap-3 transition cursor-pointer"
                  >
